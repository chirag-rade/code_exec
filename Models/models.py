...
class LLMModel:
    ...
    def __call__(self, messages, retrying=False):
        ...
        # print("DEBUG: ", rx)
        if (
            self.try_to_parse and self.output_schema != None
        ):  # user wants answer as json

            try:
                # raise Exception("test exception raised")
                result = (
                    CustomJSONParser(json.dumps(rx))
                    if tool_calling
                    else CustomJSONParser(rx)
                )
                    ...
                    if self.retry_with_history:
                        if self.provider in ["fireworks_api"] or not self.use_tool:
                            retry_message = HumanMessage(
                                content="You failed to output a valid/parsable JSON. Please try again"
                            )
                        else:
                            retry_message = HumanMessage(
                                content="You failed to call the function/tool properly. Please try again"
                            )

                        self.chat_history.append(retry_message)
                        self.chat_history_untouched.append(retry_message)
                    elif (
                        not self.retry_with_history and len(self.chat_history) > 0
                    ):  # remove last response from history - this will affect main chat history record
                        self.chat_history.pop(-1)  # Correctly remove the last item
                    self.retry -= 1
                    return self(
                        original_messages, retrying=True
                    )  # Recursively call __call__ with the original messages
                self.retry = self.config.get("retry", 3)
    ...
    def create_chain(self):
        ...
        ):  # aslong as there is an output schema then give the instructions
            if isinstance(self.output_schema, dict):
                if (
                    not self.use_tool
                    or self.provider == "groq_api"
                    or self.provider == "fireworks_api"
                ):  # these providers dont suppport function calling
                    ...
                    if self.provider == "openai_api":
                        ...
                        human_bool = [
                            isinstance(msg_type, HumanMessagePromptTemplate)
                            for msg_type in self.prompt_template.messages
                        ]
    ...
    def create_model(self):
        if self.provider == "openai_api":
            ...
            if (
                self.try_to_parse
                and self.output_schema != None
                and self.model
                in ["gpt-3.5-turbo-1106", "gpt-4-turbo", "gpt-3.5-turbo", "gpt-4o"]
            ):
            ...
            if self.try_to_parse and self.output_schema != None:
                (
                    ...
                    if isinstance(self.output_schema, dict)
                    else self.output_schema.model_json_schema()
                ),
            if self.model in [
                "dbrx-instruct",
    ...
    def correct_prompt_template(self, old_prompt_template):
        ...
        if not new_prompt_template:
            ...
            if (
                "format_instructions" not in new_prompt_template.input_variables
            ):  # and self.try_to_parse
                # find the last system message and insert format_instructions after  (better to come last)
                sys_bool = [
                    isinstance(msg_type, SystemMessagePromptTemplate)
                    for msg_type in new_prompt_template.messages
                ]
                if True in sys_bool:
                    # find the last True:
                    last_occurrence_index = (
                        len(sys_bool) - 1 - sys_bool[::-1].index(True)
                    )
                    # now insert the format_instructions into the last system message
                    # Copy the old one:
                    old_system_template = new_prompt_template.messages[0].dict()[
                        "prompt"
                    ]["template"]
                    new_system_template = (
                        "\n" + old_system_template + "\n{format_instructions}"
                    ...
                    new_prompt_template.messages[last_occurrence_index] = (
                        SystemMessagePromptTemplate.from_template(new_system_template)
                    )
                else:
